...
bool IsNetfx45Installed()
{
  ...
  if (RegistryGetValue(HKEY_LOCAL_MACHINE, g_szNetfx45RegKeyName, g_szNetfx45RegValueName, 0, (LPBYTE)&dwRegValue, sizeof(DWORD)))
  {
...
int GetNetfx10SPLevel()
{
  ...
  if (IsCurrentOSTabletMedCenter())
    bRegistryRetVal = RegistryGetValue(HKEY_LOCAL_MACHINE, g_szNetfx10SPxOCMRegKeyName, g_szNetfxStandardVersionRegValueName, 0, (LPBYTE)szRegValue, MAX_PATH);
  else
    bRegistryRetVal = RegistryGetValue(HKEY_LOCAL_MACHINE, g_szNetfx10SPxMSIRegKeyName, g_szNetfxStandardVersionRegValueName, 0, (LPBYTE)szRegValue, MAX_PATH);
...
int GetNetfxSPLevel(const TCHAR *pszNetfxRegKeyName, const TCHAR *pszNetfxRegValueName)
{
  ...
  if (RegistryGetValue(HKEY_LOCAL_MACHINE, pszNetfxRegKeyName, pszNetfxRegValueName, 0, (LPBYTE)&dwRegValue, sizeof(DWORD)))
  {
...
bool CheckNetfxBuildNumber(const TCHAR *pszNetfxRegKeyName, const TCHAR *pszNetfxRegKeyValue, const int iRequestedVersionMajor, const int iRequestedVersionMinor, const int iRequestedVersionBuild, const int iRequestedVersionRevision)
{
  ...
  if (bRegistryRetVal)
  {
    ...
    while (0 != pszToken)
    {
      ...
      pszToken = _tcstok_s(0, _T("."), &pszNextToken);
    }
  }
...
CString DoDetectDotNet()
{
  ...
  //MessageBox(0, szOutputString, _T(".NET Framework Install Info"), MB_OK | MB_ICONINFORMATION);
